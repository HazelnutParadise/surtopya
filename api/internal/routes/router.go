	...
	api := r.Group("/api")
	{
		...
		v1 := api.Group("/v1")
		{
			...
			agentProtected := v1.Group("/agent-admin", middleware.RequireAgentAdmin())
			{
				...
				agentProtected.GET("/deid", middleware.RequireAgentPermission("surveys.read"), agentAdminHandler.GetDeidUsage)
				agentProtected.POST("/deid/sessions/start", middleware.RequireAgentPermission("surveys.write"), agentAdminHandler.StartDeidSession)
				agentProtected.GET("/deid/sessions/:session_id", middleware.RequireAgentPermission("surveys.read"), agentAdminHandler.GetDeidSession)
				agentProtected.POST("/deid/sessions/:session_id/chunks/:chunk_index/annotate", middleware.RequireAgentPermission("surveys.write"), agentAdminHandler.AnnotateDeidChunk)
