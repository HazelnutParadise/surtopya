...
func setupRouter(options setupOptions) *gin.Engine {
	...
	if options.includeInternalApp {
		...
		app := api.Group("/app", middleware.RequireInternalApp())
		{
			...
			admin := app.Group("/admin", middleware.RequireAuth(), middleware.RequireAdmin())
			{
				...
				admin.GET("/users/:id", freshHandler(handlers.NewAdminHandler, (*handlers.AdminHandler).GetUser))
	...
	v1 := r.Group("/v1")
	{
		...
		agentProtected := v1.Group("/agent-admin", middleware.RequireAgentAdmin())
		{
			...
			agentProtected.GET("/surveys/:id/responses", middleware.RequireAgentPermission("surveys.read"), freshHandler(handlers.NewAgentAdminHandler, (*handlers.AgentAdminHandler).GetSurveyResponses))
			...
			agentProtected.GET("/users/:id", middleware.RequireAgentPermission("users.read"), freshHandler(handlers.NewAdminHandler, (*handlers.AdminHandler).GetUser))
