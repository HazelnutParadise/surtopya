...
func TestResponseHandler_RestartDraft_ResetsExistingDraftToCurrentPublishedVersion(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	r := gin.New()
	r.POST("/api/v1/surveys/:id/drafts/restart", func(c *gin.Context) {
		c.Set("userID", respondentID)
		h.RestartDraft(c)
	})

	req := httptest.NewRequest(http.MethodPost, "/api/v1/surveys/"+surveyID.String()+"/drafts/restart", bytes.NewReader([]byte(`{}`)))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
