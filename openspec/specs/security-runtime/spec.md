...
## Purpose
Define production safety defaults (JWT verification, CORS) and runtime readiness checks.
...
The backend SHALL expose a readiness endpoint that verifies the database is reachable.

#### Scenario: DB not reachable
- **WHEN** `GET /api/v1/ready` is called
- **AND** the database is not reachable
- **THEN** the backend responds with 503

#### Scenario: DB reachable
- **WHEN** `GET /api/v1/ready` is called
- **AND** the database is reachable
- **THEN** the backend responds with 200
