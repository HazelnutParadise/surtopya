...
- **前端內部 API（不版本化，/api/app）**
  - `POST /api/app/surveys/:id/responses/start` - 開始填答
  - `POST /api/app/responses/:id/answers` - 提交單一答案
  - `POST /api/app/responses/:id/submit` - 提交所有答案
  - `POST /api/app/surveys/:id/responses/submit-anonymous` - 匿名送出填答
  - `POST /api/app/surveys/:id/drafts/start` - 建立草稿
  - `POST /api/app/drafts/:id/answers` - 草稿儲存答案
  - `POST /api/app/drafts/:id/answers/bulk` - 草稿批次儲存答案
  - `POST /api/app/drafts/:id/submit` - 草稿送出
  - `POST /api/app/responses/claim-anonymous-points` - 匿名點數認領
  - `POST /api/app/responses/forfeit-anonymous-points` - 匿名點數放棄
...
### D. API 分層
- 公開 API 維持版本化（`/api/v1/*`）。
- 前端內部 API 不版本化（`/api/app/*`），由 BFF 代打並搭配內部簽章驗證。
